...
- **定时自动采集**：通过 APScheduler 支持 Cron 表达式配置定时任务，默认每天凌晨 2 点自动采集
...
浏览器打开 `http://localhost:3000`，进入**鉴权设置**页面，添加华为云租户的鉴权信息：
...
| `HWCloud_SCHEDULER_CRON` | `0 0 2 * * ?` | 定时任务 Cron 表达式（秒 分 时 日 月 周） |
...
系统使用 APScheduler 执行定时数据采集，默认 Cron 表达式为 `0 0 2 * * ?`（每天凌晨 2 点执行）。
...
可通过环境变量 `HWCloud_SCHEDULER_CRON` 自定义执行时间，例如：

```bash
# 每天凌晨 3 点执行
export HWCloud_SCHEDULER_CRON="0 0 3 * * ?"

# 每 6 小时执行一次
export HWCloud_SCHEDULER_CRON="0 0 0/6 * * ?"
```
